...
contract("Test Scenario 2: Sudent Enrolment in Multiple Rounds", accounts => {
  let uni = null;
  let chief = undefined;
  let admin = undefined;
  ...
  it('should handle round 2', async () => {
    // admin starts a 5 second bidding round
    await uni.startBiddingRound(5, {from: admin});
    let biddingEnd = (await uni.getBiddingEndTime()).toNumber();

    // all students bid for COMP4212
    let courseBytes8 = web3.utils.fromAscii('COMP4212');
    await assertRevert(async () => {
      await uni.makeBid(courseBytes8, 800, {from: studentA});
    }, 'student A trying to bid more tokens than they own should trigger revert');
    await uni.makeBid(courseBytes8, 800,  {from: studentB});
    await uni.makeBid(courseBytes8, 800, {from: studentC});
    await uni.makeBid(courseBytes8, 900,  {from: studentD});
    await uni.makeBid(courseBytes8, 700,  {from: studentE});
    ...
    // check only student A and student C were accepted
    let accepted = await uni.getAcceptedStudents(courseBytes8);
    assert.sameMembers(accepted, [studentB, studentC, studentD], 'incorrect students accepted to COMP4212');
    ...
    assert.equal(await uni.getBalance(studentD), 900, `studentD does not have the right number of tokens at end`);
    assert.equal(await uni.getBalance(studentE), 1800, `studentE does not have the right number of tokens at end`);
  });
});
